...
sheet_monkey_url = "https://api.sheetmonkey.io/form/f9Rr5UqsNXbtQQhLdiashz"
...
new_js_code = """        function handleFormSubmit(event) {
            ...
            // ⚠️ ĐÃ ĐỔI SANG ĐƯỜNG LINK SHEET MONKEY CỰC KỲ ỔN ĐỊNH
            ...
            // Sử dụng URLSearchParams và so khớp chính xác 100% với tên cột trên Google Sheet của bạn
            ...
            // Gửi dữ liệu về Google Sheets thông qua Sheet Monkey Webhook
            fetch(scriptUrl, {
                method: 'POST',
                mode: 'no-cors', // Tránh lỗi CORS bảo mật trên trình duyệt
                headers: {
...
for file_path in files_to_update:
    ...
    # Thay thế hàm handleFormSubmit cũ bằng phiên bản Sheet Monkey mới
    ...
    print(f"Đã tích hợp thành công Sheet Monkey cho file: {file_path}")
