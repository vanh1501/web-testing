---
name: kiem-tra-message-match
description: |
  Kiểm tra angle → Hero → Problem → Why → USP → CTA, bao gồm show-don’t-tell.
---

# Skill: kiem-tra-message-match

## Khi nào dùng

Dùng trong workflow `tao-landing-page-theo-angle` khi cần: Kiểm tra angle → Hero → Problem → Why → USP → CTA, bao gồm show-don’t-tell.
